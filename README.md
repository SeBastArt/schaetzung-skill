...
- **Customer collaboration built in**:
  ...
  - **Live offer total**: the KPI tiles, the composition bar and a summary line follow the
    ...
    and the project-management share scales proportionally (percentage editable on the page)
  ...
  - **Excel export** (.xlsx, no library): filterable position table with three-point values,
    chosen value, A/K/X and comments, the "chosen PT" column holds only offered positions (K/X values move to their own column), SUM rows for positions,
